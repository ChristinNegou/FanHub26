...
export default function WatchPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const [filters, setFilters] = useState<BarFilters>({ radius: 15, sortBy: 'distance' });
  ...
  // Load upcoming matches for the filter dropdown
  useEffect(() => {
    fetch('/api/matches')
      .then((r) => r.json())
